...
3. **Relay mode:** After the current phase, **always** append a copy-paste block:

   - **Role** / **Skill file** path under `.agents/skills/`
   - **Initiative** path
...
7. **End state:** Deliver branch/commit summary, validation (`validate-agent-log` if files changed), and a short **human UAT checklist**.

Do not pretend Cursor spawned separate agents; describe behavior as **one or more sessions** following the playbook.
