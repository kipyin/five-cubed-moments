...
- Xcode 26 or later (defaults in [`gracenotes-dev.toml`](gracenotes-dev.toml) use iPhone 17 Pro @ `OS=latest` and iPhone SE (3rd generation) @ `OS=latest`—each picks the newest installed iOS runtime for that device; override in TOML if your Xcode installs differ)
...
- `grace ci --profile full` – Lint, tests on iPhone 17 Pro, UI smoke on iPhone SE (3rd generation) @ `OS=latest` per `gracenotes-dev.toml`.
...
| When | What runs |
|------|-----------|
| **Pull request → `main`** | **Lint & build (iPhone 17 Pro)** — `grace ci` (default profile **`lint-build`**; no tests). **`CI_SIMULATOR_PRO`** / **`CI_SIMULATOR_XR`** use **`OS=latest`** for **iPhone 17 Pro** and **iPhone SE (3rd generation)** (newest installed runtime per device on the runner). |
