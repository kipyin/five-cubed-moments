...
Execute phases in order. For each phase, adopt ONLY that role’s skill from `.agents/skills/<skill-slug>/SKILL.md` (e.g. `strategize`, `architect`, `design`, `build`, `test`, `qa-review`, `vc`) and edit ONLY the files that role owns (see roles-index). After each phase, write handoff fields: Decision, Open Questions, Next Owner.
