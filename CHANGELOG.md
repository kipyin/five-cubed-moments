...
- Past: **Search journal** stays on the Past tab — the field and results are inline instead of opening a separate search screen; clearing the query restores the usual Past cards. While the field is focused, a trailing control clears non-empty text (same as many search fields) and dismisses the keyboard, with layout animation aligned to **Today** chip-to-composer timing (snappy ~0.22s; respects Reduce Motion).
