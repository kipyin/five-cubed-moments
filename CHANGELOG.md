...
- Journal: post-Seed journey (**C**) is **version-free**—eligibility uses `hasSeenPostSeedJourney`, Today completion **≥ Seed**, and `completedGuidedJournal` (skip congratulations only). Removed `OrientationReleaseGate` / `pending051*` launch cohort logic; `JournalOnboardingProgress.migrateLegacyPostSeedOrientationFlagsIfNeeded` clears legacy `pending051*` UserDefaults on launch. Settings **App tour** sets `hasSeenPostSeedJourney` on finish without completing guided journal. `AppLaunchVersionTracker` still records last marketing/bundle only.
