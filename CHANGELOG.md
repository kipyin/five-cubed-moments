...
### Changed
- Review > Insights: weekly **Reflection rhythm** uses a redesigned column visualization (completion tier per day, horizontal scroll with edge feathering when the week is wider than the card); each day is tappable and opens **Today** for that calendar day via `JournalScreen` (#115). VoiceOver hint clarifies that the control opens the journal entry for that day.
